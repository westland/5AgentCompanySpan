...
echo "PATH: $PATH"
echo "Global node bin folder: $(npm bin -g 2>/dev/null || npm config get prefix 2>/dev/null)/bin"
echo "Searching for openclaw:"
find / -name "openclaw" -type f 2>/dev/null || true
find / -name "openclaw" -type l 2>/dev/null || true
echo "---"
