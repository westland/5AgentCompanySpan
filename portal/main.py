...
def get_system_status():
    ...
    for proc in psutil.process_iter(["pid", "name", "cmdline", "create_time", "memory_info"]):
        try:
            name_match = "openclaw" in (proc.info["name"] or "").lower()
            cmdline_match = any("openclaw" in (arg or "").lower() for arg in (proc.info["cmdline"] or []))
...
@app.get("/api/cron")
def get_cron_jobs():
    db_path = os.path.join(OPENCLAW_HOME, "state", "openclaw.sqlite")
